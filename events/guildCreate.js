...
module.exports = async (client, guild) => {
    ...
    await Prefix.findOne({guildID: guild.id}, (err, prefix) => {
        ...
        if (!prefix) {
            ...
            console.log(`The guild: '${guild}' has been added to the prefix database`);
        }
        ;
    });
   await PerGuildLogandWelcome.findOne({guildID: guild.id}, (err, perguildlogandwelcome) => {
     ...
     if(!perguildlogandwelcome) {
       ...
       console.log(`The guild: '${guild}' has been added to the per guild logging, welcoming and leaving database`);
     }
   })
};
